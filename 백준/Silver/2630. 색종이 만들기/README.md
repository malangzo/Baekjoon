...
- 자료구조/알고리즘: 분할 정복 및 재귀 함수

## 🔍 문제의 핵심 포인트

문제를 이해하는 데 중요한 부분
- 시작점을 기준으로 해당 영역의 색이 같은지 먼저 확인
- 색이 다를 경우 분할 정복을 재귀적으로 실행
- 분할된 각 영역의 색이 모두 같을 경우 결과 저장

## ⏱️ 코드의 시간복잡도

- 시간복잡도: 각 분할마다 모든 칸을 한 번씩 확인하므로 재귀적 과정에서 N^2개의 칸을 확인 → O(N^2)

## 🧠 코드의 공간복잡도

- 공간복잡도: 입력받은 색종이 크기 → O(N^2)
