...
문제를 이해하는 데 중요한 부분
> 스택을 이용해 괄호의 균형 여부 판단
- 여는 괄호 있을 경우 스택에 추가
- 이후 닫힌 괄호 나올 경우 스택에 여는 괄호가 있는지 확인한 후 제거
- 닫힌 괄호가 먼저 나올 경우 불균형이므로 스택에 추가
- 결론적으로 스택이 비어 있을 경우, 해당 문자열이 균형임을 판단 가능

## ⏱️ 코드의 시간복잡도

- 시간복잡도: 문자열의 길이 n에 비례 → O(n)

## 🧠 코드의 공간복잡도

- 공간복잡도: 스택에 쌓이는 괄호 개수에 비례 → O(n)
